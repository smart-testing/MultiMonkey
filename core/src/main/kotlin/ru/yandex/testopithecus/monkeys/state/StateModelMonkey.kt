...
import ru.yandex.testopithecus.stateFilter.Filter
...
class StateModelMonkey(private val cvServerAddress: String? = null) : Monkey {
    ...
    private val filter: Filter? = if (useScreenshots) Filter(cvServerAddress!!) else null
    ...
    private fun filterStates(uiState: UiState): UiState {
        return filter?.filterStates(uiState)!!
    }
